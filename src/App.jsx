...
function ExerciseGuide({ exerciseId, name }) {
  ...
  const technique = info?.technique || 'Movimiento controlado y rango de movimiento completo y cómodo.'
  return <><button type="button" className="guide-button" onClick={(event) => { event.stopPropagation(); const target = document.getElementById(`guide-${exerciseId}`); target?.showModal?.() }}><CircleHelp size={13} /> Guía</button><dialog id={`guide-${exerciseId}`} className="guide-dialog"><div className="guide-dialog-content"><div className="modal-header"><div><div className="eyebrow">GUÍA TÉCNICA</div><h2>{name}</h2></div><button type="button" className="icon-button" onClick={() => document.getElementById(`guide-${exerciseId}`)?.close()} aria-label="Cerrar guía"><X size={17} /></button></div><div className="guide-details-box"><div className="guide-detail-row"><span>💪 Músculo trabajado</span><b>{muscle}</b></div><div className="guide-detail-row"><span>🎯 Técnica y ejecución</span><p>{technique}</p></div></div><a className="guide-search" href={`https://www.google.com/search?q=${encodeURIComponent(`${name} técnica ejercicio`)}`} target="_blank" rel="noreferrer"><MoveUpRight size={14} /> Buscar demostración en video</a></div></dialog></>
}
...
function Profile({ restSeconds, setRestSeconds, notify, logs, routines, setRoutines, setLogs, library, setLibrary, user, firebaseAuthError = false }) {
