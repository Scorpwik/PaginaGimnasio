...
function Today({ routines, logs, onSaveLog, restSeconds, notify, onGoRoutines, unit = 'kg' }) {
  ...
  const updateExerciseRest = (exerciseId, value) => setDraft((items) => items.map((exercise) => exercise.exerciseId === exerciseId ? { ...exercise, restSeconds: Math.max(0, Number(value) || 0) } : exercise))
  const finish = () => {
    if (!routine) return
...
function Routines({ routines, setRoutines, notify, user, unit = 'kg' }) {
...
function BodyMetricChart({ metrics, unit = 'kg', onEdit, onDelete }) {
  ...
  return <div className="body-chart-scene" onPointerMove={selectByClientX} onPointerDown={selectByClientX}>
    ...
    <div className="body-history-list">{records.slice().reverse().map((item, index) => {
...
function ExerciseManager({ routines, setRoutines, notify, unit = 'kg', onClose }) {
  ...
  const updateExercise = (exerciseId, key, value) => setRoutines((items) => items.map((item) => item.id !== routine.id ? item : { ...item, exercises: item.exercises.map((exercise) => exercise.exerciseId === exerciseId ? { ...exercise, [key]: value } : exercise) }))
  const removeExercise = (exerciseId) => setRoutines((items) => items.map((item) => item.id !== routine.id ? item : { ...item, exercises: item.exercises.filter((exercise) => exercise.exerciseId !== exerciseId).map((exercise, index) => ({ ...exercise, order: index })) }))
  const addExercise = (libraryItem) => {
    if (!routine || routine.exercises.some((exercise) => exercise.exerciseId === libraryItem.id)) return
    setRoutines((items) => items.map((item) => item.id !== routine.id ? item : { ...item, exercises: [...item.exercises, { exerciseId: libraryItem.id, name: libraryItem.name, targetSets: libraryItem.defaultSets || 3, targetReps: libraryItem.defaultReps || '10', targetWeight: libraryItem.defaultWeight || 0, restSeconds: libraryItem.defaultRestSeconds || 90, order: item.exercises.length }] }))
    setQuery('')
    notify('Ejercicio añadido')
  }
  const createCustom = () => {
    if (!query.trim() || !routine) return
    addExercise({ id: `custom-${Date.now()}`, name: query.trim(), defaultSets: 3, defaultReps: '10', defaultWeight: 0, defaultRestSeconds: 90 })
  }
  const results = LIBRARY.filter((item) => item.name.toLowerCase().includes(query.toLowerCase()) && !routine?.exercises.some((exercise) => exercise.exerciseId === item.id))
  return <Modal title="Editor de ejercicios" onClose={onClose} wide><div className="form-grid"><label>Rutina<select value={routine?.id || ''} onChange={(event) => setSelectedRoutineId(event.target.value)}>{routines.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label>Añadir ejercicio<input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar o crear..." /></label></div>{query && <div className="library-box compact-library">{results.slice(0, 5).map((item) => <button className="library-result" key={item.id} onClick={() => addExercise(item)}><span className="mini-icon">{item.emoji}</span><span><b>{item.name}</b><small>{item.muscleGroup}</small></span><Plus size={16} /></button>)}{!results.length && <button className="library-result" onClick={createCustom}><span className="mini-icon"><Sparkles size={14} /></span><span><b>Crear "{query}"</b><small>Ejercicio personalizado</small></span><Plus size={16} /></button>}</div>}<div className="editor-exercises global-exercise-editor">{routine?.exercises.map((exercise, index) => <div className="editor-exercise" key={exercise.exerciseId}><div className="editor-exercise-name"><span>{String(index + 1).padStart(2, '0')}</span><input value={exercise.name} onChange={(event) => updateExercise(exercise.exerciseId, 'name', event.target.value)} /></div><div className="editor-fields"><label>Series<input type="number" min="1" value={exercise.targetSets} onChange={(event) => updateExercise(exercise.exerciseId, 'targetSets', Number(event.target.value))} /></label><label>Reps<input value={exercise.targetReps} onChange={(event) => updateExercise(exercise.exerciseId, 'targetReps', event.target.value)} /></label><label>{unit.toUpperCase()}<input type="number" min="0" value={displayWeight(exercise.targetWeight, unit)} onChange={(event) => updateExercise(exercise.exerciseId, 'targetWeight', parseWeight(event.target.value, unit))} /></label><label>Descanso<select value={exercise.restSeconds || 90} onChange={(event) => updateExercise(exercise.exerciseId, 'restSeconds', Number(event.target.value))}><option value="60">60s</option><option value="90">90s</option><option value="120">2 min</option><option value="180">3 min</option></select></label></div><div className="editor-row-actions"><IconButton label="Eliminar ejercicio" onClick={() => removeExercise(exercise.exerciseId)}><Trash2 size={14} /></IconButton></div></div>)}</div><div className="modal-actions"><Button onClick={onClose}><Save size={16} /> Listo</Button></div></Modal>
}
