...
import { INITIAL_LOGS, INITIAL_ROUTINES, LIBRARY, formatDate, formatDay, getBest, getStreak, getVolume, readStore, repsBase, today, weekDay, writeStore } from './lib/data'
...
function Today({ routines, logs, onSaveLog, restSeconds, setRestSeconds, notify, onGoRoutines }) {
  ...
  const finish = () => {
    if (!routine) return
    onSaveLog({ id: existing?.id || `log-${Date.now()}`, routineId: routine.id, date: today(), dayLabel: routine.name.split('·')[0].trim(), durationMinutes: existing?.durationMinutes || 60, completed: true, exercises: draft.map(({ exerciseId, name, sets }) => ({ exerciseId, name, sets: sets.filter((set) => set.status !== 'pendiente') })) })
...
function WorkoutEditor({ log, routines, onClose, onSave, onDelete }) {
  const routineExercises = routines.flatMap((routine) => routine.exercises)
  const availableExercises = [...new Map(routineExercises.map((exercise) => [exercise.exerciseId, exercise])).values()]
  const [form, setForm] = useState(() => ({ ...log, durationMinutes: log.durationMinutes || 60, exercises: log.exercises.map((item) => ({ ...item, sets: item.sets.map((set, index) => ({ ...set, setNumber: set.setNumber || index + 1 })) })) }))
