...
function BodyMetricChart({ metrics, unit = 'kg', onEdit, onDelete }) {
  ...
  return <div className="body-chart-scene" onPointerMove={selectByClientX} onPointerDown={selectByClientX}>
    <svg className="body-chart-svg" viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="none">
     ...
     <button type="button" className="history-button body-history-toggle" onClick={() => setHistoryOpen((value) => !value)}><History size={14} /> {historyOpen ? 'Ocultar historial' : `Ver historial (${records.length})`}<ChevronRight size={14} className={historyOpen ? 'rotate-90' : ''} /></button>
...
function Profile({ restSeconds, setRestSeconds, notify, logs, routines, setRoutines, setLogs, library, setLibrary }) {
